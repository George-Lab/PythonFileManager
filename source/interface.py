...
tk.Button(glb.root, text="Folder Up", command=fnc.folderUp).grid(sticky="NSEW", column=0, row=0)
# Keyboard shortcut for going up
glb.root.bind("<Alt-Up>", fnc.folderUp)

tk.Button(glb.root, text="Add File or Folder", command="").grid(
    sticky="NSEW", column=0, row=1, rowspan=1
)

tk.Button(glb.root, text="Copy", command=fnc.copyFile).grid(
    sticky="NSEW", column=0, row=2, rowspan=1
)

tk.Button(glb.root, text="Cut", command=fnc.cutFile).grid(
    sticky="NSEW", column=0, row=3, rowspan=1
)

tk.Button(glb.root, text="Delete", command=fnc.deleteFile).grid(
    sticky="NSEW", column=0, row=4, rowspan=1
)

tk.Button(glb.root, text="Insert", command=fnc.insertFile).grid(
    sticky="NSEW", column=0, row=5, rowspan=1
)

tk.Button(glb.root, text="View Mode", command=fnc.changeViewMode).grid(
...
tk.Entry(glb.root, textvariable=glb.currentPath).grid(
